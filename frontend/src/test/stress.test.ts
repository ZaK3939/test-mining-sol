...
describe('Stress Tests - Production Readiness', () => {
  ...
  describe('Concurrent User Operations', () => {
    it(
      'should handle concurrent user initializations',
      async () => {
    ...
    it(
      'should handle concurrent facility purchases',
      async () => {
    ...
    it(
      'should handle rapid sequential API calls',
      async () => {
        ...
        logger.info(
          `✅ Rapid API calls: ${successCount}/${callCount} succeeded, avg ${averageTime.toFixed(
            2
          )}ms per call`
        );
      },
  ...
  describe('Memory and Performance Tests', () => {
    it(
      ...
      async () => {
        ...
        logger.info(
          `✅ Memory pressure test: ${successCount}/${iterations} succeeded, avg ${averageTime.toFixed(
            2
          )}ms`
        );
      },
    ...
    it(
      'should demonstrate cache efficiency under load',
      async () => {
        ...
        const { duration: coldTime } = await TestHelpers.measurePerformance(
          () =>
            Promise.all(
              Array.from({ length: operationsPerRound }, () =>
                user.client.fetchUserState(user.keypair.publicKey)
              )
            ),
        ...
        logger.info(
          `✅ Cache efficiency: ${(improvementRatio * 100).toFixed(1)}% improvement with cache`
        );
      },
      TEST_CONSTANTS.TIMEOUTS.DEFAULT
    );
    ...
    it(
      'should handle network instability',
      async () => {
          ...
          while (attempts < maxAttempts) {
            try {
              ...
              await new Promise((resolve) =>
                setTimeout(resolve, TEST_CONSTANTS.NETWORK.RETRY_DELAY)
              );
            }
          }
        ...
        const avgAttempts =
          results.reduce((sum, r) => sum + (r?.attempts || 1), 0) / results.length;

        logger.info(
          `✅ Network resilience: ${(analysis.successRate * 100).toFixed(
            1
          )}% success, avg ${avgAttempts.toFixed(1)} attempts`
        );
      },
      TEST_CONSTANTS.TIMEOUTS.DEFAULT
    );
  ...
  describe('Resource Management', () => {
    it(
      'should efficiently manage batch operations',
      async () => {
        ...
        const { duration: individualTime } = await TestHelpers.measurePerformance(async () => {
        ...
        logger.info(
          `✅ Batch efficiency: ${(improvement * 100).toFixed(1)}% faster than individual calls`
        );
      },
      TEST_CONSTANTS.TIMEOUTS.DEFAULT
    );
    ...
    it(
      ...
      async () => {
        ...
        logger.info(
          `✅ Memory cleanup: ${largeOperationCount} operations completed, cleanup successful`
        );
      },
      TEST_CONSTANTS.TIMEOUTS.LONG
    );
